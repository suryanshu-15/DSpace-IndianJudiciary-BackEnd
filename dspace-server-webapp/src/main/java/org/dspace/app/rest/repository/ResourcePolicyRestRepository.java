...
public class ResourcePolicyRestRepository extends DSpaceRestRepository<ResourcePolicyRest, Integer>
    ...
    protected ResourcePolicyRest createAndReturn(Context context) throws AuthorizeException, SQLException {
        ...
        if (resourcePolicy != null) {
            ...
            resourcePolicy.setPageStart(resourcePolicyRest.getPageStart());
            resourcePolicy.setPageEnd(resourcePolicyRest.getPageEnd());
            resourcePolicy.setDownload(resourcePolicyRest.isDownload());
            resourcePolicy.setPrint(resourcePolicyRest.isPrint());
            resourcePolicyService.update(context, resourcePolicy);
            UUID affectedId = null;
            if (groupUuidStr != null && !groupUuidStr.isEmpty()) {
                affectedId = UUID.fromString(groupUuidStr);
            } else if (epersonUuidStr != null && !epersonUuidStr.isEmpty()) {
                affectedId = UUID.fromString(epersonUuidStr);
            }
            if (affectedId != null) {
                roleAuditLogService.logRoleChange(context, affectedId, "PERMISSION_GRANT", "Policy");
...
//    }
    ...
    protected void patch(Context context, HttpServletRequest request, String apiCategory, String model, Integer id,
        ...
        // Apply standard patch operations (only for supported fields)
        resourcePatch.patch(obtainContext(), resourcePolicy, patch.getOperations());

        // Now apply custom fields manually
        ResourcePolicyRest resourcePolicyRest = converter.toRest(resourcePolicy, utils.obtainProjection());

        // Update pageStart and pageEnd if present
        Integer patchedPageStart = resourcePolicyRest.getPageStart();
        if (patchedPageStart != null) {
            resourcePolicy.setPageStart(patchedPageStart);
        }

        Integer patchedPageEnd = resourcePolicyRest.getPageEnd();
        if (patchedPageEnd != null) {
            resourcePolicy.setPageEnd(patchedPageEnd);
        }

        if (resourcePolicyRest.getStartDate() != null) {
            resourcePolicy.setStartDate(resourcePolicyRest.getStartDate());
        }
        if (resourcePolicyRest.getEndDate() != null) {
            resourcePolicy.setEndDate(resourcePolicyRest.getEndDate());
        }
